  ...
  <main role="main">
    ...
    <article>
      <header>
        ...
        <div class="meta">
	   <div class="tags">Reading time: <?php echo $article->text()->readingtime() ?> | </div>	
          <time datetime="<?php echo $article->date('c') ?>"><?php echo $article->date('F dS, Y'); ?></time>
          <?php if($article->tags() != ''): ?> |
          <ul class="tags">
          <?php foreach(str::split($article->tags()) as $tag): ?>
      ...
      <a href="<?php echo $article->url() ?>">[<?php echo l::get('readMore');?> →]</a>
	<?php $aWord=l::get('readLater');?>
	<?php echo readlater($article->title(), 'instapaper', '['.$aWord.' →]');?>
	</p>
    </article>
